...
Read the README before you start here. Then take a look at the below instructions and examples, both in the PDF and in the source code, to get an idea of how to use this template. Delete this chapter before your final submission.

\subsection{Content}
\label{sec:content}

Refer to the kickoff slides for instructions about the aims of the seminar and the requirements on the report. Be clear and precise, deliver substance and understanding in your own words. Every sentence in your text should have a clear purpose, avoiding irrelevant information.

This is a general template, you can adjust its structure if you have a good reason to do so (it may not fit all papers). Don't adjust the formatting. Target and do not exceed 12 pages, plus references. Ensure proper citations.

\subsection{Sections}
\label{sec:sections}

You can begin new sections with the \texttt{\textbackslash section} command. This goes down over \texttt{\textbackslash subsection} to the \texttt{\textbackslash subsubsection} level.
...
Insert cross-references to other sections, figures, equations, etc. with the \texttt{\textbackslash cref} command. This includes not only the number but also the text part, for example \cref{sec:sections}, \cref{fig:label}, \cref{tab:label}, \cref{eq:label}, \cref{list:label2} or \cref{alg:label}.
...
For bold symbols, use the \texttt{\textbackslash symbf} command that works for both Latin and Greek letters like $\symbf{x}$ and $\symbf{\phi}$ (as opposed to \texttt{\textbackslash mathbf}). It also fixes some alignment issues, e.g. $\tilde{\symbf{A}}$ versus $\tilde{\mathbf{A}}$. Similarly, you can use \texttt{\textbackslash symcal} or \texttt{\textbackslash symbb} and other commands instead of their \texttt{\textbackslash math...} counterparts.
...
Source code listings can be included from files with the \texttt{\textbackslash lstinputlisting} command or directly inside \texttt{lstlisting} environments. You can specify captions and labels as well.

\lstinputlisting[caption={Caption.},label={list:label}]
  {listings/hello-world.cpp}

\begin{lstlisting}[caption={Caption.},label={list:label2}]
print("Hello World!")
\end{lstlisting}
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}[b]{0.66\textwidth}
    \centering
    \input{graphics/template-instructions/surface.tex}
    \caption{Caption B}
    \label{fig:label_b}
  \end{subfigure}
  \caption{Caption.}
